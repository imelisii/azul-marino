...
export class PrinterService {
  ...
  async createProfessionalCarnet(carnetData: CarnetData): Promise<ArrayBuffer> {
    ...
    doc.addImage(this.base64Logo, 'PNG', 100, -10, 50, 50);

   
    doc.setFontSize(25);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(primaryColor);
    doc.text('Carnet de socio', 55, 10, { align: 'center'  });
    ...
    doc.text(vencimiento, 45, 60);
    doc.addImage(qrImage, 'PNG', 12, 65, 25, 25);
